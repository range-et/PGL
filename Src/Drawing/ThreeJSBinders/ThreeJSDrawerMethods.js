...
function DrawCylinderBasedVertices(graph, bounds, divisons){
  const ds = divisons || 1;
  const Cgroup = AddCylinderBasedImaging(graph.nodes, ds, bounds);
  return Cgroup;
}

// draw cylinders where required
function AddCylinderBasedImaging(vertexMap, divisonLength, bounds) {
  // returns a group
  const group = new THREE.Group();
